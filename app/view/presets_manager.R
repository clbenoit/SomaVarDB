                         ...
                         fluidRow(box(
                           ...
                           column(width = 4,
                                  selectInput(ns("separatorManifest"),"Choose a field separator" ,
                                              choices = c("Tabulation" = "\t", "comma" = ",", "space" = " ", "semicolon" = ";")),
                                  helpText("How are your columns separated in your input file")
                           ),
...
server <- function(id, con, appData, genomicData, main_session) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$confirmremove, priority = 100, {
      ...
      dbSendQuery(conn = appData$con, paste0("DELETE FROM presets WHERE user = '", Sys.getenv("SHINYPROXY_USERNAME"), "' AND name = '", input$selectset, "' ;"))
      removeModal()
    })
    ...
    observeEvent(c(input$selectset,
                   ...
                   appData$filters$manifest#,
                     ...
                     if(input$selectset == 'In use filter values'){
                       ...
                       reactiveValuesInputsInside$impact <- appData$filters$impact
                       reactiveValuesInputsInside$trlist <- appData$filters$trlist
    ...
    check_format <- function(data) {
      if (all(c("Transcripts", "Genes") %in% colnames(data))) {return(TRUE) } else { return(FALSE)}}
    ...
    observeEvent(input$confirmManifest, {
      ...
      req(file_dataManifest())
      removeModal()
      dbExecute(conn = appData$con, "CREATE TABLE IF NOT EXISTS manifests_list (user_id TEXT, manifests TEXT);")
